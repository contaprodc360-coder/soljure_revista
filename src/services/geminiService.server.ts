...
2. UNA SOLA ESTRUCTURA DE CONCLUSIONES, RECOMENDACIONES Y REFERENCIAS: El documento final resultante debe tener estrictamente una única sección de "## Conclusiones" (numeradas de manera unificada y continua, mínimo 5 en total), una única sección de "## Recomendaciones" (numeradas de manera unificada y continua, mínimo 5 en total) y una única sección de "## FUENTES BIBLIOGRÁFICAS Y JURISPRUDENCIALES DE CONSULTA". Fusiona y enriquece de forma unificada el contenido preexistente con los nuevos descubrimientos de tu ampliación. Las fuentes de consulta bibliográficas y jurisprudenciales deben estar consolidadas en esta única y sola sección final.
...
5. REFERENCIAS EN APA 7 Y RIGOR HUMANO: Sigue de principio a fin las normas APA de 7.ª edición para las citas intratextuales y la sección bibliográfica final, unificando ambas bajo el título "## FUENTES BIBLIOGRÁFICAS Y JURISPRUDENCIALES DE CONSULTA". Redacta de forma plenamente natural pero con el más selecto nivel oratorio y dogmático, eliminando clichés o modismos automatizados. No te identifiques como IA.
6. FUENTES BIBLIOGRÁFICAS REALES ECUATORIANAS: Al finalizar el tema, incluya una única y unificada sección titulada '## FUENTES BIBLIOGRÁFICAS Y JURISPRUDENCIALES DE CONSULTA' que cite autores doctrinales reales de alta academia andina (v.gr., Dr. Jorge Zavala Baquerizo, Dr. Hernán Salgado Pesantes, Dr. Alfonso Barrera Valverde) y números reales vigentes de Gacetas Judiciales o del Registro Oficial de la República del Ecuador. Esta debe ser la única sección de fuentes de consulta bibliográficas y jurisprudenciales del artículo.
...
function cleanExpandedContent(content: string, title: string = "", isExpansion: boolean = true): string {
  ...
  return consolidateFinalSections(cleaned.trim());
}
...
function consolidateFinalSections(content: string): string {
  ...
  // If there are multiple trailing empty paragraphs or back-to-back headings, clean them
...
export async function generateFullEditorial(topic: string, area: ExpertiseArea, customInstructions?: string): Promise<{ title: string, summary: string, managerSummary: string, content: string }> {
  ...
  ⚠️ CRÍTICO - REQUISITO DE EXTENSIÓN MÁXIMA: El artículo debe ser EXTREMADAMENTE DETALLADO Y EXTENSO, con una longitud equivalente de entre 10 y 15 PÁGINAS impresas (un mínimo aproximado de entre 3,500 y 5,000 palabras reales). Para cumplir este objetivo de extensión sin redundancias, desarrolla de manera exhaustiva cada una de las secciones requeridas, incorporando múltiples párrafos de desarrollo doctrinal profundo, debates teóricos minuciosos, desgloses normativos precisos del Ecuador, análisis histórico-jurídico, debates forenses prácticos y una amplia y rica exégesis.
  ...
  El contenido científico del campo 'content' debe estructurarse STRICTLY bajo los siguientes títulos, numerados y ordenados de la siguiente forma, asegurando que cada sección sea altamente extensa para alcanzar el rango de 10-15 páginas (3,500 - 5,000 palabras):
  ...
  ## Abstract
  (Versión académica en inglés del resumen redactada con precisión de traducción jurídica).
  
  ## Keywords
  (Versión en inglés de las palabras clave).
  
  ## 1. Introducción
  (Sección muy extendida: Contextualice ampliamente el problema jurídico en el escenario jurídico del Ecuador, justifique con detalle el interés social, académico y científico de la investigación, plantee el problema práctico que enfrentan los operadores de justicia y delimite el objetivo general del estudio).
  
  ## 2. Marco Doctrinal
  (Sección exhaustiva de alta extensión: Desarrolle la doctrina nacional e internacional pertinente, abordando la evolución teórica del instituto jurídico, los principios procesales asociados y los debates dogmáticos actuales entre tratadistas clásicos y modernos con absoluto rigor intelectual).
  
  ## 3. Marco Normativo
  (Sección exhaustiva de alta extensión: Realice un análisis normativo minucioso de cada disposición constitucional, ley orgánica, ordinaria o reglamento aplicable en el Ecuador. Debe incluir al menos una tabla de contrastación técnica Markdown detallando diferencias sustanciales, plazos procesales improrrogables o excepciones previas).
  
  ## 4. Metodología de Análisis Cualitativo o Dogmático
  (Explique minuciosamente el diseño metodológico de la investigación dogmático-jurídica, de alcance descriptivo-explicativo, con un enfoque eminentemente cualitativo y técnicas dogmáticas de interpretación literal, sistemática y teleológica basadas en bases de datos indexadas).
  
  ## 5. Análisis Crítico del Caso
  ### 5.1 Antecedentes
  ### 5.2 Hechos relevantes
  ### 5.3 Pretensiones
  ### 5.4 Argumentos de las partes
  ### 5.5 Motivación judicial
  ### 5.6 Análisis crítico de seguridad jurídica (sección muy extensa que evalúa de forma analítica y objetiva la viabilidad y rectitud del fallo o criterio estándar aplicable según la jurisprudencia de las altas cortes ecuatorianas, confrontándolo con el principio de seguridad jurídica y la tutela judicial efectiva).
  
  ## 6. Ratio Decidendi
  (Sección detallada: Desarrolle de forma extensa la razón de la decisión o el núcleo argumental vinculante, sustentado directamente en preceptos constitucionales y jurisprudencia obligatoria del Ecuador).
  
  ## 7. Conclusiones
  (Debe haber un mínimo de 5 conclusiones adjetivas y sustantivas sólidas, profundas, numeradas de 1 a 5).
  
  ## 8. Recomendaciones
  (Debe haber un mínimo de 5 recomendaciones procesales sustanciales de compliance y patrocinio forense práctico para profesionales, numeradas de 1 a 5).
  
  ## FUENTES BIBLIOGRÁFICAS Y JURISPRUDENCIALES DE CONSULTA
  (Lista de referencias bibliográficas y precedentes jurisprudenciales completa, unificada en esta única sección final bajo formato APA 7, incluyendo el Registro Oficial, Resoluciones del Pleno CNJ, sentencias CC y CNJ, doctrinas académicas andinas reales y repositorios universitarios de alto rigor. Debe haber EXACTAMENTE una sección de esta bibliografía al final, unificada y sin duplicaciones).
